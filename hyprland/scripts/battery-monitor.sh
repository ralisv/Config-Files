...
upower --monitor | while read -r line; do
    ...
    if [ $status != $new_status ] && [ $new_status = "Full" ]; then
        notify-send -t 5 "Battery fully charged"
    fi

    status=$new_status

    # Send notifications if battery is low
    if [ $status = "Discharging" ]; then
        if [ $capacity -le 20 ]; then
            notify-send -t 10 "Battery low" "Battery is at $capacity%"
        elif [ $capacity -le 10 ]; then
            notify-send -t 100 "Battery critically low" "Battery is at $capacity%"
        fi
    fi
